...
import { FileCode2, FolderCode, Badge, Zap } from 'lucide-react';
      ...
      <Container className="relative z-10">
          ...
          <div className="flex justify-center gap-4 mb-12 flex-wrap">
            {showcaseTabs.map((tab) => (
              <button
                ...
                className={`px-6 py-3 rounded-lg font-medium font-mono transition-all ${
                  activeTab === tab.id
        ...
        <FadeIn delay={0.6}>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 mt-20 max-w-4xl mx-auto">
            {[
              { label: 'Lines of Code', value: '847+', icon: FileCode2 },
              { label: 'Files Generated', value: '43', icon: FolderCode },
              { label: 'Quality Score', value: '96%', icon: Badge },
              { label: 'Deploy Time', value: '3.7m', icon: Zap }
            ].map((stat, index) => (
              <div key={stat.label} className="text-center">
                <stat.icon className="w-10 h-10 mx-auto mb-3 text-primary-400" />
                <div className="text-3xl font-bold font-mono gradient-text mb-2">{stat.value}</div>
